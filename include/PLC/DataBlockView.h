...
public:
  ...
  S7BigEndianToLittleEndian(std::vector<uint8_t> &Sourcebuffer,
                 ...
                 const OPCUAModernDataStruct &var);
...
class DeviceReader
{
  public:
    ...
    Result<bool, RichError> batchWriteOPCUABlock_ToPLC(DataBlock *data);
...
public:
    ...
    std::vector<OPCUAModernDataStruct>& getVariabeDataVector() { return m_variable_vector; }
    ...
    OPCUAModernDataStruct *getVariable(const std::string &path);
    ...
    getVariableMap(std::vector<OPCUAModernDataStruct> &m_variable_vector);
    ...
    void
    updateBufferFromS7ModernStructByMSB(std::vector<OPCUAModernDataStruct> &vector);
    void
    updateBufferFromS7ModernStructByLSB(std::vector<OPCUAModernDataStruct> &vector);
...
private:
    ...
    int calculateOffset(const OPCUAModernDataStruct& var) const;
