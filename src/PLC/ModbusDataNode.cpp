...
// ==================== 构造函数 ====================
ModbusDataNode::ModbusDataNode(ModbusDataStruct *var)
    : m_dirty (false) {
  var = var;
}
...
// ==================== 读取操作实现 ====================
Result<ValueType, RichError> ModbusDataNode::readValue() {
    if (!m_dirty ) {
        return Result<ValueType, RichError>(RichError{"error cache"});
    } else {
        return Result<ValueType, RichError>(QVariant(var->cached_value));
    }
}

// Result<bool, RichError> ModbusDataNode::readValueFromPLC() {
//     // TODO: 实现从 PLC 读取的逻辑
//     auto result = reader.lock()->batchReadNode(m_dataRootNode);
//     emitBatchData();
...
//   auto result = reader.lock()->batchWriteNode(m_dataRootNode);
//   emitBatchData();
//   return Result<bool, RichError>(result);
// }
...
//     records.reserve(m_dataRootNode.size()); // 预分配内存
    
//     for (const auto& var : m_dataRootNode) {
