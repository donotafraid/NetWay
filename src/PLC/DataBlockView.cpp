...
void DataBlock::updateS7ModernStructFromBuffer()
{
    ...
    {
        auto result = DataTypeMapper::TransformBytesToSpecial(item.data_type_enum,m_variableDataBuffer,item.bytes_offset,item.s7_data_type_length,item.bit_offset);
...
void DataBlock::updateBufferFromS7ModernStructByMSB(std::vector<OPCUAModernDataStruct> &vector)
{
...
void DataBlock::updateBufferFromS7ModernStructByLSB(std::vector<OPCUAModernDataStruct> &vector)
{
   for(auto &VariableItem : vector) 
   {
     switch (VariableItem.data_type_enum) {
...
Result<bool, RichError> DataBlock::isArray(OPCUAModernDataStruct &data_var) {
...
Result<bool, RichError> DeviceReader::batchWriteOPCUABlock_ToPLC(DataBlock *data) {
  auto result = m_opcUA->ensureConnection();
  if(result.is_fail())
  {
    return Result<bool,RichError> (result);
...
QVariant DataBlockModel::data(const QModelIndex &index, int role) const {
  ...
  std::vector<OPCUAModernDataStruct> &items = m_dataBlock->getVariabeDataVector();
...
bool DataBlockModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    ...
    std::vector<OPCUAModernDataStruct>& items = m_dataBlock->getVariabeDataVector(); // 注意需要非const版本
    OPCUAModernDataStruct& item = items[index.row()];
...
void DataBlockModel::batchSetDataForS7() {
  ...
  for (auto &item : m_dataBlock->getVariabeDataVector()) {
    Result<NormalDataType, RichError> result =
        DataTypeMapper::TransformBytesToSpecial(
...
void DataBlockController::onViewWriteRequested() {
  if (m_reader->getConnectWayForS7()) {
    ...
    auto result = m_reader->batchWriteOPCUABlock_ToPLC(m_DataBlock.get());
    if (result.is_fail()) {
